...
public struct BigDecimal : IComparable, IComparable<BigDecimal>
{
    ...
    /// </summary>
    public const int Precision = 110;
    ...
    {
        if (bigDecimal < 0)
            return new BigDecimal(bigDecimal.Mantissa * (-1), bigDecimal.Exponent);

        return bigDecimal;
    }

    public static BigDecimal PowBig(BigDecimal basis, int exponent)
    {
        if (exponent == 0)
            return 1;
